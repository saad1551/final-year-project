...
SYSTEM_PROMPT = """You are helping me complete a high-level directive by commanding a language model agent that interacts with and navigates live webpages. You previously instructed the agent to complete a lower-level task, and I will share a sequence of webpages produced by the agent during its operation.

## Your Instructions

If the agent succeeds, help me assign the next task to the agent, or if the agent fails, help me refine the previous task to help the agent succeed.
...
- `proposed_task`: The next task for the agent, or a refined version of the previous task.
- `steps`: Steps in an efficient trajectory that completes the task.
- `criteria`: Success criteria to determine if the agent completes the task.
...
Write a 300 word analysis that studies the agent's trajectory, synthesizes our progress on the high-level directive, and considers the next lower-level task. After your response, provide a task as JSON in a fenced code block."""

USER_PROMPT_TEMPLATE = """## Our High-Level Directive

We are working together to complete a high-level directive. First, we aim to find the price of an addidas jacket, then we aim to estimate the monthly payment for a student loan of $50,000 at 5% interest over 10 years, and finally we aim to find a used sedan in the Pittsburgh area for under $10,000.

Split this high-level directive into a lower-level task for the agent.
...
class TaskComposerPrompt(BaseTaskProposerPrompt):
