...
SYSTEM_PROMPT = """You are helping me explore a website by commanding a language model agent that interacts with and navigates live webpages. We previously instructed the agent to explore one aspect of the website, and I will share a sequence of webpages produced by the agent during its operation.

## Your Instructions

Based on the agent's trajectory, help me discover content and features that may interest real users.
...
- `proposed_task`: Command the agent to explore specific content and features on the website that may interest real users.
- `steps`: Steps in an efficient trajectory that explores this aspect of the website.
- `criteria`: Specific answers and criteria to determine if the agent explores this aspect of the website.
...
In this example, we instructed the agent to explore an event page on `liveevents.iadb.org` for the IDB Annual Meetings to discover official hotels and requirements for official delegations.
...
In this example, we instructed the agent to explore a page on `boldtcastle.com` with information about visiting Boldt Castle to discover operating dates, admission prices, and how to get to Heart Island.
...
In this example, we instructed the agent to explore a page on `visitwestchesterny.com` that lists various coffee houses in Westchester County to discover their names, addresses, and links to their descriptions.
...
In this example, we instructed the agent to explore the 'Reservations' page on `odetterestaurant.com` to discover policies for dietary accommodations, birthdays, a deposit requirement, cancellations, and rescheduling.
...
In this example, we instructed the agent to explore a form on `dottyabouticecream.co.uk` to discover the corporate event hiring process for Dotty's ice cream van, and encountered fields for event details, guest count, and flavor inquiries.
...
In this example, we instructed the agent to explore a page on `engineered.polestar.com` to find specifications about Polestar Engineered Optimization for specific Volvo models, including the 2023 Volvo XC60 with a B5 Drive-E engine.
...
In this example, we instructed the agent to explore a page on `ajga.org` about Performance Based Entry (PBE) Stars for junior golfers to discover how they carry over to the next season and tips for maximizing tournament opportunities.
...
In this example, we instructed the agent to explore a page on `passports.gov.au` with a section about the documents needed to prove Australian citizenship for individuals born in Australia on or after August 20, 1986.
...
Based on the agent's trajectory, help me discover content and features on {website} that may interest real users.
...
- `proposed_task`: Command the agent to explore specific content and features on {website} that may interest real users.
- `steps`: Steps in an efficient trajectory that completes the task.
