...
MAX_TRAJECTORY_STEPS = (
    20  # Balanced: enough steps for complex tasks, fits in 8GB VRAM with 4-bit LoRA
)
MAX_HISTORY_STEPS = 2
...
PROFILE_STEPS = False  # Controlled by --profile flag
...
class StepProfiler:
    """Accumulates per-phase wall-clock timings across all steps in a trajectory.

    Phases tracked:
        observation  — get_observation_with_retry() incl. configured sleep
        markdown     — convert_html_to_markdown()
        inference    — generate_action_from_observation()
        action_exec  — client.action()

    Usage (inside the step loop)::

        profiler = StepProfiler()

        t0 = time.perf_counter()
        obs = get_observation_with_retry(client)
        profiler.record("observation", t0)

        ...

        profiler.print_trajectory_summary()
    """

    PHASES = ["observation", "markdown", "inference", "action_exec"]

    def __init__(self):
        self.timings: dict[str, list[float]] = {p: [] for p in self.PHASES}

    def record(self, phase: str, t_start: float) -> float:
        """Record elapsed time since t_start for the given phase.

        Returns the elapsed time so callers can log it immediately if desired.
        """
    ...
    def print_step_summary(self, step: int) -> None:
        ...
        for phase in self.PHASES:
            vals = self.timings[phase]
            if vals:
                parts.append(f"{phase}={vals[-1]:.3f}s")
        total = sum(self.timings[p][-1] for p in self.PHASES if self.timings[p])
        print(
            f"[PROFILE] Step {step:>2d}  " + "  ".join(parts) + f"  total={total:.3f}s"
        )

    def print_trajectory_summary(self) -> None:
        """Print mean/min/max per phase across all recorded steps."""
        import statistics

        sep = "─" * 68
        completed = len(self.timings["observation"]) or len(self.timings["markdown"])
        n = max(len(v) for v in self.timings.values()) if self.timings else 0
        if n == 0:
            print("[PROFILE] No steps recorded.")
            return

        print(f"\n{sep}")
        print(f"  TRAJECTORY PROFILING SUMMARY  ({n} step{'s' if n != 1 else ''})")
        print(sep)
        print(f"  {'Phase':<16}  {'mean':>7}  {'min':>7}  {'max':>7}  {'total':>8}")
        print(f"  {'-' * 14}  {'-' * 7}  {'-' * 7}  {'-' * 7}  {'-' * 8}")
        ...
        for phase in self.PHASES:
            ...
            if not vals:
                continue
            note = "  (incl. configured sleep)" if phase == "observation" else ""
            note += "  (GPU)" if phase == "inference" else ""
            print(
                f"  {phase:<16}  "
            ...
            if phase != "inference":
                if not cpu_combined:
                    cpu_combined = list(vals)
                else:
                    cpu_combined = [
                        cpu_combined[i] + vals[i]
                        for i in range(min(len(cpu_combined), len(vals)))
                    ]
        ...
        if cpu_combined:
            ...
            print(
                f"  {'CPU total':<16}  "
                f"{statistics.mean(cpu_combined):>6.3f}s  "
                f"{min(cpu_combined):>6.3f}s  "
                f"{max(cpu_combined):>6.3f}s  "
                f"{sum(cpu_combined):>7.3f}s"
                f"  (excl. inference)"
            )
        print(sep)
...
def initialize_browser_session(start_url):
    browser_config = BrowserConfig(
        playwright_url=BROWSER_SERVER_URL, screen_width=1920, screen_height=1080
    )
...
def generate_action_from_observation(
    tokenizer, model, task_instruction, current_observation, markdown_content, history
):
    import gc
...
def parse_action_from_response(response_text, agent_prompt):
    ...
    for match in matches:
        try:
            ...
            candidate_action = agent_prompt.parse_action(
                f"```json\n{candidate_json}\n```"
            )
...
def run_trajectory(
    ...
    model,
    tokenizer,
    trainer: OnPolicyTrainer = None,
    enable_rl_update: bool = True,
    ...
    if enable_rl_update and trainer is None:
        ...
        if rl_config.algorithm == "sb3_ppo":
            ...
            print(
                "   The custom PPO algorithm (--algorithm ppo) computes gradients on actual trajectories"
            )
            ...
            print(
                f"RL Trainer initialized with {rl_config.algorithm.upper()} algorithm."
            )
    ...
    try:
        ...
        for step in range(MAX_TRAJECTORY_STEPS):
            ...
            pipeline_debug(
                f"Step {step + 1} - Response length: {len(response_text)} chars"
            )
            pipeline_debug(
                f"Step {step + 1} - Response preview: {response_text[:100]}..."
            )
            ...
            if profiler:
                profiler.record("action_exec", _t0)
            if status == BrowserStatus.ERROR:
            ...
            current_observation = get_observation_with_retry(client)
            if profiler:
                _elapsed = profiler.record("observation", _t0)
                profiler.print_step_summary(step + 1)
        ...
        try:
            with open("latest_trajectory_data.json", "w") as f:
                ...
                print(
                    "Saved trajectory data for judgment to latest_trajectory_data.json"
                )
        except Exception as e:
        ...
        if torch.cuda.is_available():
            torch.cuda.empty_cache()
            print(
                f"GPU memory cleared. Allocated: {torch.cuda.memory_allocated() / 1024**3:.2f} GB"
            )
        ...
        if enable_rl_update and trainer is not None and len(trajectory_prompts) > 0:
            ...
            pipeline_debug(
                f"  Judgment: success={judgment.success}, efficiency={judgment.efficiency}, self_correction={judgment.self_correction}"
            )
            for i, (p, r) in enumerate(zip(trajectory_prompts, trajectory_responses)):
                pipeline_debug(
                    f"  Step {i + 1}: prompt={len(p)} chars, response={len(r)} chars"
                )
            pipeline_debug("=" * 50)
            ...
            print(
                f"Trajectory Reward: {rl_update_stats.get('trajectory_reward', 0):.4f}"
            )
            ...
            if "kl_divergence" in rl_update_stats:
                print(
                    f"KL Divergence (vs old policy): {rl_update_stats['kl_divergence']:.4f}"
                )
            if "ref_kl_divergence" in rl_update_stats:
                print(
                    f"KL Divergence (vs SFT model): {rl_update_stats['ref_kl_divergence']:.4f}"
                )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Run browser navigation with on-policy RL training"
    )
    parser.add_argument(
        "--num_trajectories",
        type=int,
    ...
    parser.add_argument(
        "--save_every", type=int, default=5, help="Save checkpoint every N trajectories"
    )
    parser.add_argument(
        "--resume_from",
        type=str,
    ...
    parser.add_argument(
        "--start_idx", type=int, default=0, help="Starting index in the dataset"
    )
    ...
    parser.add_argument(
        "--grpo_group_size", type=int, default=4, help="GRPO group size for comparison"
    )
    parser.add_argument(
        "--grpo_beta", type=float, default=0.1, help="GRPO KL divergence coefficient"
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Enable per-step phase profiling (observation/markdown/inference/action_exec)",
    )
    ...
    current_module.PROFILE_STEPS = args.profile

    if args.debug:
        print("[DEBUG] Debug mode enabled for both pipeline and RL trainer")

    if args.profile:
        print("[PROFILE] Step profiling enabled")
    ...
    if args.resume_from and enable_rl_update:
        ...
        if args.algorithm == "sb3_ppo":
            ...
            print(
                "   Consider using --algorithm ppo instead for meaningful RL learning.\n"
            )
        ...
        print(
            f"  Total updates from checkpoint: {trainer.training_stats.get('total_updates', 0)}"
        )
        print()
    ...
    if args.resume_trajectory is None and args.resume_dataset_idx is None:
        if checkpoint_metadata and checkpoint_metadata.get("trajectory_id") is not None:
            ...
            last_dataset_index = checkpoint_metadata.get(
                "dataset_index", args.start_idx + last_trajectory_id - 1
            )
            ...
            print(
                f"Resuming from trajectory {resume_trajectory_offset + 1}, dataset index {resume_start_idx}"
            )
            print()
    ...
    if remaining_trajectories <= 0:
        print(
            f"All {args.num_trajectories} trajectories already completed in checkpoint."
        )
        print(f"\n=== Training Complete ===")
        sys.exit(0)
    ...
    for i in range(remaining_trajectories):
        ...
        print(f"\n{'=' * 60}")
        print(
            f"TRAJECTORY {trajectory_num}/{args.num_trajectories} (Dataset Index: {task_idx})"
        )
        print(f"{'=' * 60}")
        ...
        if trajectory_result:
            ...
            if trajectory_result["judgment"] is not None:
                ...
                print(
                    f"Trajectory {trajectory_num} produced no steps; skipping observability log."
                )
            ...
            if trajectory_result.get("rl_update_stats"):
                ...
                print(
                    f"Average Reward (last 10): {sum(total_rewards[-10:]) / len(total_rewards[-10:]):.4f}"
                )
            ...
            if trainer and trajectory_num % args.save_every == 0:
                ...
                checkpoint_path = (
                    f"{args.checkpoint_dir}/checkpoint_trajectory_{trajectory_num}"
                )
