...
class TrainingLogger:
    ...
    def log_trajectory(
        ...
        if judgment is not None:
            ...
            self_correction = judge_self_correction if judge_self_correction is not None else 0.0
        ...
        row = {
            ...
            "kl_divergence": round(kl_divergence, 6) if kl_divergence is not None else None,
            "ref_kl_divergence": round(ref_kl_divergence, 6) if ref_kl_divergence is not None else None,
        ...
        print(f"[LOG] Trajectory {trajectory_id}: reward={reward_str}, "
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        skip_reasons = df[df["used_for_optimization"] == False]["skip_reason"].value_counts().to_dict()
        ...
        summary = {
            ...
            "optimization_rate": round(used_for_opt / total * 100, 1) if total > 0 else 0,
            "skip_reasons": skip_reasons,
        }
        
        if len(completed_df) > 0:
            summary["avg_reward"] = round(completed_df["reward"].mean(), 4)
            summary["avg_judge_success"] = round(completed_df["judge_success"].mean(), 4)
    ...
    def print_summary(self):
        ...
        print(f"Used for optimization: {summary.get('used_for_optimization', 0)} "
